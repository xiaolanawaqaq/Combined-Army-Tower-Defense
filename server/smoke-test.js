...
async function main(){
  ...
  try{
    ...
    // hello 只承诺协议版本；rulesVersion 必须与服务端 RULES_VERSION 一致（当前 1）
    const serverRules = require("../shared/game-types").RULES_VERSION;
    if(a.hello.protocolVersion!==1 || a.hello.rulesVersion!==serverRules) throw new Error("hello version mismatch");
    ...
    send(a,"upgradeWeapon",{});
    await waitFor(a,x=>x.state.weaponUpgradeLevel===1,"A global weapon upgrade");
