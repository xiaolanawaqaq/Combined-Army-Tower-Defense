...
function roomView(room,viewerId){
  return {
    ...
    players:[...room.players.values()].map(p=>({id:p.id,name:p.name,ready:!!p.ready,connected:!!p.connected,abandoned:!!p.abandoned,
      record:(room.stats&&room.stats[p.id])||null})),
...
function attachTransport(server, dbReady){
  ...
  function sendState(room){
    ...
    broadcast(room,"state",p=>{
      const ps=publicState(room.session,p.id);
      // 双人对战：把查看者的账号总战绩带给客户端（HUD/结算页显示）
      if(room.gameMode==="versus" && room.stats) ps.record=room.stats[p.id]||{wins:0,losses:0};
      return {state:ps};
    });
  }
  // 拉取房间里已登录玩家的对战战绩缓存（无数据库/游客则缺省）
  function loadStats(room){
    if(!dbReady) return;
    const progressRepo=require("./repositories/progress-repo");
    for(const p of room.players.values()){
      if(!p.userId) continue;
      progressRepo.getRecord(p.userId,"versus").then(rec=>{
        room.stats=room.stats||{};
        if(room.players.has(p.id)) room.stats[p.id]=rec;
        updateRoom(room);
      }).catch(()=>{});
    }
  }
  // 对战结算记账：胜者记 win、败者记 lose，并刷新战绩缓存
  function creditVersusResult(room){
    const session=room.session;
    if(!session || room.gameMode!=="versus" || !dbReady) return;
    const winnerIdx=session.result==="win"?0:(session.result==="lose"?1:-1);
    const seats=[...room.players.values()].slice(0,2);
    if(winnerIdx<0 || seats.length<2) return;
    const winnerPlayer=seats[winnerIdx], loserPlayer=seats[1-winnerIdx];
    if(!winnerPlayer.userId || !loserPlayer.userId) return;
    const progressRepo=require("./repositories/progress-repo");
    const idemKey=`versus:${room.code}:${session.seed}`;
    progressRepo.recordVersusResult(winnerPlayer.userId,loserPlayer.userId,session.level||1,idemKey)
      .then(()=>{ loadStats(room); })
      .catch(e=>console.error("versus stat error:",e.message));
  }
    ...
    async function handleMessage(ws, msg){
      try{
        ...
        if(msg.type==="create"){
          ...
          if(prevPlayer && prevPlayer.userId) player.userId=prevPlayer.userId;
          room=rooms.create(player);
          // 设置游戏模式（versus = 双人对战，限 2 人）
          if(msg.mode==="independent") room.gameMode="independent";
          else if(msg.mode==="versus") room.gameMode="versus";
          ensureSession(room);
          loadStats(room);
        ...
        if(msg.type==="setBossEnabled"){
          if(room.hostId!==player.id) throw new Error("只有房主可以设置 Boss");
          const state=ensureSession(room);
          command(state,player.id,{type:"setBossEnabled",value:msg.value});
        ...
        if(msg.type==="startWave"){
          // 双人对战：任何一方摆好了都能开战；其它模式仍限房主
          if(room.gameMode!=="versus" && room.hostId!==player.id) throw new Error("只有房主可以开始进攻");
          const state=ensureSession(room);
          command(state,player.id,{type:"startWave"});
          sendState(room); return;
        }
        if(msg.type==="deploy"||msg.type==="move"||msg.type==="merge"||msg.type==="upgradeWeapon"||msg.type==="upgradeRate"||msg.type==="upgradeArmor"||msg.type==="upgradeDefense"||msg.type==="chooseMapEvent"||msg.type==="useCoopSkill"||msg.type==="useUltimate"||msg.type==="toggleUltLoadout"||msg.type==="awaken"||msg.type==="chooseEndlessChest"){
          const state=ensureSession(room);
          command(state,player.id,msg);
          sendState(room); return;
        }
        if(msg.type==="rematch"){
          ...
          // 联机会话金币是局内的：重开照旧清零。
          // 局外金币钱包在客户端本地持久化（session_store.gd arena_coins），失败重开费由客户端扣。
          room.session=createSession([...room.players.keys()], Date.now()>>>0, room.gameMode||"coop");
          room.session.hostId=room.hostId;
          for(const p of room.players.values()){
            p.ready=false;
  ...
  const interval=setInterval(()=>{
    try{
      ...
      for(const room of rooms.roomsByCode.values()){
        ...
        if(room.session.over) creditVersusResult(room);
        sendState(room);
      }
    }catch(e){
