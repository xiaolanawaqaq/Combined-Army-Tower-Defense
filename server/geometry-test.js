...
const {
  ...
  INDEPENDENT_PATHS,INDEPENDENT_SLOT_META,INDEPENDENT_SLOT_COUNT,WAVE_START_DELAY,
...
assert.strictEqual(SLOT_COUNT,SLOT_META.length);
assert(SLOT_META.length>32);
...
expectInvalid(()=>command(coop,"c",{type:"deploy",troopId:coop.trays.c[0].id,targetSlot:coop.slots.length}));

const independent=startSession("independent");
const lastInd=independent.paths[0].slots.length-1;
for(let i=0;i<3;i++){
  const id=independent.players[i].id;
  command(independent,id,{type:"deploy",troopId:independent.trays[id][0].id,targetSlot:i?lastInd:0});
}
assert(independent.paths[0].slots[0]);
assert(independent.paths[1].slots[lastInd]);
assert(independent.paths[2].slots[lastInd]);
expectInvalid(()=>command(independent,"a",{type:"deploy",troopId:independent.trays.a[0].id,targetSlot:independent.paths[0].slots.length}));
const view=publicState(independent,"a");
assert.strictEqual(view.pathViews.length,3);
assert.strictEqual(view.pathViews[1].slots.length,independent.paths[1].slots.length);
assert(view.pathViews[1].slots[lastInd]);

command(independent,"a",{type:"startWave"});
step(independent,WAVE_START_DELAY+1.1);
